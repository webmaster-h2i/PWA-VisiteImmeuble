    ...
    return(
        <div>
            <div className="flex justify-center m-9">
                <h3 className="text-lg text-[color:var(--first-text-color)]">Récapitulatif</h3>
            </div>
            <div className="p-2">
            ...
            <div className="flex justify-center mt-12 mr-2 ml-2 mb-5">
                <button className="w-full text-[color:var(--first-text-color)] bg-sky-600 rounded-md py-2 px-4 hover:bg-[color:var(--first-button-color)] m-1" onClick={() => {window.location.href="/element"}}><i><ArrowLeft className="w-5 inline mr-1 mb-1"/></i>Ajout d'élém.</button>
                <button className="w-full text-[color:var(--first-text-color)] bg-sky-600 rounded-md py-2 px-4 hover:bg-[color:var(--first-button-color)] m-1" onClick={() => {window.location.href= osDeclarant ? "/declarant":"/signatures"}}>{osDeclarant ? "Déclarant":"Signatures"}<i><ArrowRight className="w-5 inline ml-1 mb-1"/></i></button>
            </div>
        </div>
    )
}
...
const TableauRecap = ({listElement, idVisite, setLoading, loading, setListELement}) => {
    ...
    if(listElement.length > 0){
        ...
        return(
            <div className="divide-y divide-orange-600 bg-neutral-800 rounded-md p-1">
                <div className="grid grid-cols-7">
                    <div className="flex justify-center text-[color:var(--first-text-color)] text-sm pt-1 pb-2 col-span-2">
                        Secteur
                    </div>
                    <div className="flex justify-center text-[color:var(--first-text-color)] text-sm pt-1 pb-2 col-span-2">
                        Composant
                    ...
                    <div className="flex justify-center text-[color:var(--first-text-color)] text-sm pt-1 pb-2 col-span-1">
                        action
                    </div>
                </div>
                {listElement.map((elem, index) =>
                    <div key={index} className="grid grid-cols-7">
                        <div className="flex justify-center text-[color:var(--first-text-color)] text-xs pt-3 col-span-2">
                            {elem.secteur.nom}
                        </div>
                        <div className="flex justify-center text-[color:var(--first-text-color)] text-xs pt-3 col-span-2">
                            {elem.composant.nom}
                        </div>
                        <div className="flex justify-center text-[color:var(--first-text-color)] text-xs pt-1 col-span-2">
                        ...
                        <div className="inline-flex justify-center text-xs pt-2 col-span-1">
                            <div className="p-1">
                                <button className=" text-[color:var(--first-text-color)] rounded-full shadow-2xl" onClick={() => window.location.href="/element/"+elem.secteur.id+"/"+elem.composant.id}>
                                    <Pen className="w-4 text-sky-500"/>
                                </button>
                            </div>
                            <div className="p-1">
                                <button className="text-[color:var(--first-text-color)] rounded-full shadow-2xl" onClick={(e) => handleDeleteElement(e, elem.secteur.id, elem.composant.id)}>
        ...
        return(
            <div className='flex justify-center text-[color:var(--first-text-color)] text-sm'>
                <h5>Aucun élément</h5>
            </div>
        )   
    }
