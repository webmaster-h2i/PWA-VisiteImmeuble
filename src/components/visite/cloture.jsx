        ...
        <div>
            ...
            <menu>
                <div className="flex justify-center mt-12 mr-3 ml-3">
                    ...
                    <button className="w-full text-white text-sm  hover:bg-sky-700 rounded-md py-2 px-4 m-1" onClick={(e) => handleCloture(e)}>Clôturer</button>
                </div>
            </menu>
        </div>
    )
...
const ListeEnvoiEmail = ({listePersonne, setListePersEmail, listePersEmail, setDialogContent}) => {
    ...
    return(
        <div>
            <div className="flex justify-center m-5">
                <h3 className="text-sm text-white">Participants auquels envoyer le rapport</h3>
            </div>
            ...
            <menu>
                <div className="flex justify-center mt-12 mr-3 ml-3">
                    <button className="w-full text-sm text-white  hover:bg-sky-700 rounded-md py-2 px-4 m-1" value="close">Annuler</button>
                    <button className="w-full text-white text-sm  hover:bg-sky-700 rounded-md py-2 px-4 m-1" onClick={() => setDialogContent("modifEmail")}>Envoyer</button>
                </div>
            </menu>
        </div>
    )
        ...
        <div>
            ...
            {listePersEmail.map((personne, index) =>
                <div className="mt-2" key={index}>
                    <label htmlFor="email" className="block mb-2 text-sm font-medium text-white">{personne.nom}</label>
                    <input onBlur={(e) => personne.email = e.target.value} type="email" name="email" id="email" placeholder="email" defaultValue={!personne.email ? '':personne.email} className="border text-xs rounded-lg block w-full p-1.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white"/>
                </div>
            )}
            <menu>
                <div className="flex justify-center mt-12 mr-3 ml-3">
                    <button className="w-full text-sm text-white  hover:bg-sky-700 rounded-md py-2 px-4 m-1" value="close">Annuler</button>
                    <button className="w-full text-white text-sm  hover:bg-sky-700 rounded-md py-2 px-4 m-1" onClick={handleModifEmail}>Clôturer</button>
                </div>
            </menu>
        </div>
    )
